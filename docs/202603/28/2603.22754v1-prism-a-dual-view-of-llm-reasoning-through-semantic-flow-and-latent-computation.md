...
## 摘要
大语言模型（LLMs）通过生成多步推理轨迹来解决复杂问题。然而，这些轨迹通常仅从两个视角之一进行分析：生成文本中跨不同推理步骤的 Token 序列，或者单一步骤内跨模型层的隐藏状态向量。我们提出了 PRISM（通过语义和隐式建模进行概率推理检查），这是一个用于联合分析这两个层级的框架和诊断工具，为推理如何跨步骤和跨层演化提供了统一的视角。在多个推理模型和基准测试中，PRISM 揭示了推理过程中的系统性模式，表明失败的轨迹更有可能陷入无意义的验证循环，并进一步分化为“过度思考”和“过早承诺”等不同模式，这些模式在得出候选答案后的表现各不相同。它进一步揭示了提示词（prompting）如何通过改变语义转换和内部计算模式，在综合准确率之外重塑推理行为。通过将推理轨迹建模为结构化过程，PRISM 使这些行为变得可观察和可分析，而不仅仅依赖于最终任务的准确率。综上所述，这些见解使 PRISM 成为分析和诊断大语言模型推理过程的实用工具。
