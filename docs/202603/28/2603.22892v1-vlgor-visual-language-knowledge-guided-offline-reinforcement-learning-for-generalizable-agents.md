...
tldr: 本研究针对大语言模型在强化学习中缺乏物理感知、泛化能力受限的问题，提出了VLGOR框架。该框架通过微调视觉语言模型（VLM），结合初始视觉观察和高层指令生成具有时空一致性的想象回放数据。利用反事实提示增强数据多样性，使智能体在离线强化学习中更好地将语言指令与环境感知对齐。实验证明，该方法在机器人操纵任务中显著提升了对未知任务的泛化性能。
selection_source: fresh_fetch
motivation: 现有大语言模型在强化学习中缺乏对物理环境的直接感知，导致其难以理解环境动态并泛化至新任务。
method: 提出VLGOR框架，通过微调视觉语言模型生成基于反事实提示的想象回放数据，以丰富离线强化学习的训练样本。
result: "在机器人操纵基准测试中，VLGOR在需要新策略的未知任务上比基准方法成功率提高了24%以上。"
conclusion: 结合视觉与语言知识生成高质量想象数据，能有效增强离线强化学习智能体的环境对齐能力和任务泛化性。
---
