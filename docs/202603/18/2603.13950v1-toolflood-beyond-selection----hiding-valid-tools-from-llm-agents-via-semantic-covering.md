...
tags: ["query:rl-tl"]
score: 7.0
evidence: 针对工具增强型LLM智能体的检索层攻击
tldr: "本研究针对LLM智能体的工具检索阶段提出了一种名为ToolFlood的攻击方法。不同于以往针对工具选择的攻击，ToolFlood通过在嵌入空间中注入少量精心设计的对抗性工具，利用语义覆盖策略占据检索结果的前k位，从而将所有良性工具挤出智能体的上下文。该方法结合了LLM生成的元数据与贪心选择算法，在仅注入1%恶意工具的情况下，于ToolBench等基准上实现了高达95%的攻击成功率，揭示了当前工具增强型系统在检索层面的严重安全漏洞。"
selection_source: fresh_fetch
motivation: 现有研究多关注工具选择阶段的攻击，而忽视了嵌入式检索层面对工具增强型LLM智能体潜在的安全威胁。
method: 通过LLM生成多样化的工具元数据，并利用迭代贪心算法在嵌入空间中选择能最大化覆盖用户查询的对抗性工具。
result: "在ToolBench等标准基准测试中，ToolFlood仅需1%的注入率即可达到最高95%的攻击成功率，有效屏蔽了良性工具。"
conclusion: 检索层面的语义覆盖攻击对LLM智能体构成严重威胁，揭示了嵌入式检索系统在规模化应用中的鲁棒性缺陷。
---
