...
## 摘要
在基准测试上对大语言模型（LLMs）进行直接评估可能会产生误导，因为相对较强的性能可能反映的是对任务的熟悉程度，而非实际能力。“先训练后测试”（train-before-test）方法通过在评估前为每个模型提供任务相关的训练（最初通过监督微调实现）来控制任务熟悉度。然而，合适的训练数据往往难以获得，且评估结果会随所选数据的不同而变化。在本文中，我们提出了一种用于“先训练后测试”的两阶段测试时强化学习（RL）对齐方法。首先，利用单样本的强化学习实现模型与任务格式的初步对齐；其次，利用带有多数投票奖励的测试时强化学习使模型与基准分布对齐。我们的测试时强化学习对齐方法的效果与基于 SFT 的“先训练后测试”相当，但不需要特定任务的训练集。在一个没有训练数据的特定领域基准测试中，我们表明直接评估低估了基础模型，这些模型在对齐后表现显著提升，从而实现了对其能力更真实的评估。此外，对于推理任务，微调模型与其基础模型之间的性能差距在对齐后基本消失，这表明文献中报道的许多来自 RLVR/SFT 的增益并非推理能力的差异，而是任务熟悉度的伪影。
