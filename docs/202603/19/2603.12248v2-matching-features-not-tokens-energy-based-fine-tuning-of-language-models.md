...
tldr: 针对传统交叉熵训练仅关注逐词预测而忽视序列整体行为的问题，本文提出了一种基于特征匹配的语言模型微调方法（EBFT）。该方法通过匹配生成分布的序列级统计特征，在无需特定验证器或偏好模型的情况下提供语义反馈。EBFT利用步进块并行采样提高效率，并在编程和翻译任务中展现出优于SFT且比肩强化学习的效果，为提升模型生成质量提供了新路径。
selection_source: fresh_fetch
motivation: 传统的交叉熵训练依赖教师强制，难以直接优化模型在自主生成长序列时的整体语义表现。
method: 提出能量驱动微调（EBFT），利用步进块并行采样生成序列并匹配其特征统计量，通过策略梯度进行在线更新。
result: 在编程和翻译等任务中，EBFT的下游准确率优于SFT并比肩RLVR，同时实现了更低的验证集交叉熵。
conclusion: EBFT证明了通过匹配序列级特征而非仅预测单个Token，可以有效提升语言模型的生成性能和语义一致性。
---
