...
title_zh: 面向大语言模型智能体的模式优先工具 API：关于工具误用、恢复及预算性能的受控研究
authors: "Akshey Sigdel, Rista Baral"
date: 2026-03-12
pdf: "https://arxiv.org/pdf/2603.13404v1"
tags: ["query:rl-tl"]
score: 8.0
evidence: 研究基于模式的工具契约和结构化验证诊断是否能提高可靠性
...
motivation: 旨在探究工具接口的规范化设计（如JSON Schema）是否能提升LLM智能体在受限资源下的任务执行可靠性与容错能力。
method: 在确定性软件工程沙盒中，对比评估了自由文本、JSON Schema以及带有结构化诊断信息的Schema三种接口形式在不同预算下的表现。
result: 实验表明模式化接口有效降低了接口层面的格式误用，但未能改善语义层面的逻辑误用，且在受限本地模型下任务成功率仍不理想。
conclusion: 接口形式化能显著提升智能体对调用契约的遵守度，但语义理解质量和任务超时仍是制约本地推理模型性能的核心瓶颈。
---

## 摘要
工具使用已成为现代大语言模型（LLM）智能体的核心，但接口设计很少被作为独立的实验变量进行研究。本文探讨了基于模式（schema）的工具契约和结构化验证诊断是否能在严格的交互预算下提高可靠性。我们评估了三种保持相同工具语义和信息内容的条件：自由格式文档、JSON Schema 规范以及带有结构化诊断的 JSON Schema。我们实现了一个包含日志、指标、配置和仓库任务的确定性软件工程沙盒，并使用一个开源本地模型、三个随机种子、三种接口条件和四个预算进行了一项全交叉试点评估。我们报告了最终任务成功率、接口误用、执行失败、语义误用、恢复行为以及开销。在该试点研究中，所有条件下的成功率均为零，而模式条件减少了接口误用，但未能减少语义误用。证据支持一个精确的解释：接口形式化提高了对契约的遵守程度，但在受限的本地推理下，语义动作质量和超时敏感任务仍然是主要的瓶颈。
