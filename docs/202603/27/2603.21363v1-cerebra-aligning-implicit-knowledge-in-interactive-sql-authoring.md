...
title_zh: Cerebra：在交互式 SQL 编写中对齐隐性知识
...
## 摘要
大语言模型（LLM）驱动的工具显著降低了编写 SQL 查询的门槛。然而，用户的指令往往描述不足，默认模型理解未明确提供的隐性知识，例如数据集模式（schemas）、领域惯例和特定任务需求。这导致生成的脚本经常出错，需要用户反复澄清其意图。此外，用户难以验证生成的脚本，因为他们无法核实模型是否正确应用了隐性知识。我们提出了 Cerebra，这是一款交互式自然语言转 SQL（NL-to-SQL）工具，旨在 SQL 编写过程中对齐用户与 LLM 之间的隐性知识。Cerebra 根据用户指令自动从历史 SQL 脚本中检索隐性知识，在交互式树状视图中展示这些知识以供代码审查，并支持迭代优化以改进生成的脚本。为了评估 Cerebra 的有效性和可用性，我们对 16 名参与者进行了用户研究，结果表明该工具提升了对定制化 SQL 编写的支持。Cerebra 的源代码可在 https://github.com/zjuidg/CHI26-Cerebra 获取。
