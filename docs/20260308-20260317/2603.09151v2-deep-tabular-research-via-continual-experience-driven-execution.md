...
tags: ["query:rl-tl"]
score: 7.0
evidence: 用于操作执行和多步推理的智能体框架
tldr: 针对大语言模型在处理具有复杂层级和非规范布局的非结构化表格时面临的长程分析难题，本文提出了“深度表格研究（DTR）”框架。该框架将表格推理视为闭环决策过程，通过构建层级元图捕捉双向语义，并引入期望感知选择策略来优化执行路径。此外，利用孪生结构化记忆整合历史经验，实现了策略的持续改进。实验证明，该方法在处理复杂表格任务上具有显著优势，强调了战略规划与底层执行分离的重要性。
selection_source: fresh_fetch
motivation: 现有大语言模型在处理具有复杂层级表头和非规范布局的非结构化表格时，难以进行多步推理和长程分析。
method: 提出一种基于智能体的闭环决策框架，结合层级元图构建、期望感知路径选择以及孪生结构化记忆机制。
result: 在挑战性的非结构化表格基准测试中，该方法显著提升了推理性能，验证了其在复杂分析任务中的有效性。
conclusion: 将战略规划与底层执行分离，并结合持续的经验驱动优化，是解决长程表格推理任务的关键。
---
