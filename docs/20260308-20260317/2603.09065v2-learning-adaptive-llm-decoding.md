...
tags: ["query:rl-tl"]
score: 8.0
evidence: 通过强化学习和可验证的终端奖励进行训练
tldr: 针对大语言模型解码时固定采样参数无法适应不同任务难度的问题，本文提出了一种学习自适应解码策略的方法。通过引入轻量级解码适配器并利用强化学习进行训练，该方法能在推理时根据计算资源动态选择采样策略。研究涵盖了序列级（上下文多臂老虎机）和标记级（POMDP）两种粒度的自适应，在数学和编程基准测试中显著提升了准确率与计算预算之间的权衡效率。
selection_source: fresh_fetch
motivation: 传统的固定采样超参数无法应对不同提示词和解码步骤中任务难度及不确定性的巨大差异。
method: 引入轻量级适配器，通过强化学习在序列级和标记级动态调整采样策略，以优化给定计算预算下的生成质量。
result: "在MATH基准测试中，标记级适配器在固定预算下将Pass@1准确率提升了高达10.2%，序列级适配器也带来了2-3%的提升。"
conclusion: 学习自适应解码策略能有效优化LLM的推理效率，在保持或提升性能的同时更合理地分配计算资源。
---
