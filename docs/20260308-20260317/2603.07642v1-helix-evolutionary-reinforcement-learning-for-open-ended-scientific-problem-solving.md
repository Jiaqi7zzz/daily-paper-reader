...
## 摘要
具有推理能力的大语言模型（LLMs）在解决复杂科学问题方面展现出日益增长的潜力。然而，此类任务本质上具有领域特定性、无界性和开放性，要求在广阔且灵活的解空间中进行探索。现有方法无论是纯学习驱动还是依赖精心设计的工作流，往往面临探索效率有限和泛化性差的问题。为克服这些挑战，我们提出了 HELIX——一种结合上下文经验的分层演化强化学习框架。HELIX 引入了两个关键创新：(i) 一个多样化且高质量的候选解池，通过上下文学习扩展了探索空间；(ii) 利用强化学习进行迭代策略精炼，从而逐步提升解的质量。这种协同作用使得发现更先进的解成为可能。在圆堆积（circle packing）任务中，HELIX 仅使用 14B 模型就实现了 2.63598308 的半径之和，达到了当前最先进的水平。在标准机器学习基准测试中，HELIX 进一步超越了配备精心设计流水线的 GPT-4o，在 Adult 和 Bank Marketing 数据集上实现了 5.95 点的平均 F1 值提升。
